...
from models import db, User, Song, Album, Playlist, UserRating, UserRatingPlaylist
import os
...
@app.route('/logout')
def logout():
    session.pop('id', None) # none, cookie it remove
...
def rating():
    ...
    # SONG.RATING ==> AVG VALUE
    # Update ---> Song Table ------> (id, ...., ...., .... Rating=AvgValue)
    all_ur = UserRating.query.filter_by(song_id=song_id).all() # [<ur>,.....]
    list_rating = [int(ur.rating) for ur in all_ur] # [3,5,2,1]
    avg_rating = sum(list_rating)/len(list_rating) # SUM(3,5,2,1)/4
...
@app.route('/rating_playlist', methods=['POST'])
@user_required()
def rating_playlist():
    curr_user = get_current_user()    
    ...
    user_id = curr_user.id
    playlist_id = request.args.get('playlist_id') # song_id in url_for 
    rating = request.form.get('rating') # Name of input in Form
    # USER RATING TABLE POPLATING
    # Update/CreateNew ---> UserRating Table ---> (user_id, song_id, Rating)
    ur = UserRatingPlaylist.query.filter_by(user_id=user_id, playlist_id=playlist_id).first()
    if not ur:
        ur = UserRatingPlaylist(user_id=user_id, playlist_id=playlist_id, rating=rating)
        db.session.add(ur)
        db.session.commit()
    else:
        ur.rating = rating
    ...
    # SONG.RATING ==> AVG VALUE
    # Update ---> Song Table ------> (id, ...., ...., .... Rating=AvgValue)
    all_ur = UserRatingPlaylist.query.filter_by(playlist_id=playlist_id).all() # [<ur>,.....]
    list_rating = [int(ur.rating) for ur in all_ur] # [3,5,2,1]
    avg_rating = sum(list_rating)/len(list_rating) # SUM(3,5,2,1)/4
    ...
    playlist.rating = avg_rating
    db.session.commit()



    flash(f'Rating Added to playlist : {playlist.name}', 'success')
